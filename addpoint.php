...
$_SESSION['pcustomername']='';
  ...
  if (isset($_POST['savepoint'])){
    ...
    $p_name='';
		...
		$mypoint->add_point($p_addno,$c_code);
    $_SESSION['c_code']='';
    $_SESSION['pcustomername']='';
    unset($_GET['c_code']);
...
<table align="center">
    <tr>
        <td align="center"><font color="#ffb84d" size="14">Adding Point Customer</font></td>
    </tr>
</table>
</div>
    ...
    <tr>
      <?php
        ...
        try
          {
              $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $sql = "SELECT * FROM wpoint ORDER BY :c_order";
              $stmt = $pdo->prepare($sql);
        ...
        echo '<td ><select name="mypoint" id="mypoint" onchange="changecust();" >
        <option value="" disabled selected> Select Point Name...</option>';

        while ($row = $stmt->fetchObject()) {
              //echo $row->c_code;
              $mycode=$row->p_code;
              echo '<option value="'.$row->p_code.'">'. $row->p_name. '</option>';
