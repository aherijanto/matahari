...
function __construct($b_code,$b_refno,$b_date,$b_dateinput,$s_code,$u_code,$g_code,$i_code,$i_name,$i_qty,$i_cogs,$i_disc1,$i_disc2,$i_disc3,$fun){
    $this->b_code=$b_code;
...
//14
    ...
    function delete_purchase(){
        ...
        try {
            ...
            $stmtdelete_purchase_head = "DELETE FROM wbuyhead WHERE b_code='$b_code'";
                //echo '<br/>'.$stmt;
                $pdo->exec($stmtdelete_purchase_head);
        } catch(PDOException $e) {
            echo $e->getMessage();
        }
        try {
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmtdelete_purchase_tail = "DELETE FROM wbuytail WHERE b_code='$b_code'";
                
                $pdo->exec($stmtdelete_purchase_tail);
        } catch(PDOException $e) {
            echo $e->getMessage();
        }
...
/*--------------INSERT INTO WBUYHEAD---------------------------------------------------------------------------*/
        try {
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = "INSERT INTO wbuyhead (b_code,b_refno,b_date,b_dateinput,s_code,u_code,tag)
                VALUES('$b_code','$b_refno','$b_date','$b_dateinput','$s_code','$u_code','$myfun')";
                //echo '<br/>'.$stmt;
                $pdo->exec($stmt);
        } catch(PDOException $e) {
            echo $e->getMessage();
...
/*-------------INSERT INTO WBUYTAIL----------------------------------------------------------------------------*/
        ...
        try {
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = "INSERT INTO wbuytail (b_code,g_code,i_code,i_name,i_qty,i_cogs,i_disc1,i_disc2,i_disc3,tglexp,tag)
                VALUES('$b_code','$g_code','$i_code','$i_name','$i_qty','$i_cogs','$i_disc1','$i_disc2','$i_disc3','$expdate','origin')";
                $pdo->exec($stmt);
        } catch(PDOException $e) {
            echo $e->getMessage();
        }
