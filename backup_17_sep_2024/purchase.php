<?php
		...
		switch($_GET["action"])
		{
			...
			case "save":
				...
				if(!empty($_SESSION["cart_item"]))
				{
					...
					echo 'on save... purchase';
					echo $status_pcs;
					$purchasing = new Purchase($myinvno,$myrefno,$mydate1,$mydateon,$mysupp,$myuser,'0','0','0','0','0','0','0','0',$status_pcs);
					// print_r($purchasing);
					if($_SESSION['status_pcs'] = 'edit'){
						$purchasing->delete_purchase();
					}
					...
					foreach($_SESSION["cart_item"] as $myItem)
					{
						...
						$purchaseDetail = new Purchase($myInvNo,$myrefno,$mydate1,$mydateon,$mysupp,$myuser,$myItemCode,$myBarcode,$myItemName,$myQty,$myPrice,$myDisc1,$myDisc2,$myDisc3,'0');
	...
	if($_GET['actionmf']='editinv'){
		if(isset($_POST['btnsearchpcs'])){
			$noinvmf = $_POST['txtpcssearch'];
			if ($noinvmf == ''){
				echo 'Please input a valid invoice no ....<br/>';
				echo 'Click BACK button to process....<br/><br/>';
				echo '<a href="/purchase.php?action=new" style="color:white;background-color:#2874a6;border-radius:5px;text-decoration:none;padding:10px;margin-top:60px">Back to Purchasing...</a>';
				exit;
			}else{
				...
				if ($rowcount > 0){
					...
					echo '<label style="color:white;background-color:red;padding:10px;border-radius:5px;";>Data Not Found..</label>';
					echo '</div>';
				}
	...
	<body bgcolor="#FFFAF1">
		...
		<!-- <form action="" method="post"> -->
			...
			?>
			<div>
				<form action="/purchase.php?actionmf=editinv" method="post">
			...
			<table width="100%" font="calibri">
				<th  class="text-center">INVOICE NO</th><th class="text-center">REFERENCE NO</th><th class="text-center">INVOICE DATE</th><th class="text-center">DUE DATE</th><th class="text-center">SUPPLIER</th>
				<tr>
					<td class="text-center"><input type="text" class="form-control input-md" name="invno" value="<?php echo $_SESSION['myinvno']; ?>"  readonly/></td>

					<form method="post" action="">
						<td class="text-center"><input type="text" class="form-control input-md" name="myrefno"  onchange="this.form.submit();" value="<?php echo $_SESSION['myrefno']; ?>" /></td>
					<!-- </form>

					<form method="post" action="">	 -->
						<td><input type="date" class="form-control input-md" name="invdate" id="invdate"  onchange="this.form.submit();" value="<?php echo $_SESSION['xdate']; ?>" />
					</td>
					<!-- </form>

					<form method="post" action=""> -->
						<td><input type="date" class="form-control input-md" name="invdue"  id="invdue" onchange="this.form.submit();" value="<?php echo $_SESSION['invdue']; ?>"/></td>
						...
						<?php
							...
							echo '<td align="center"><select name="mysupp" class="form-control form-select" id="mysupp" onchange="this.form.submit();" >
		...
		<!-- </form> -->
		<br/>
		<div class="txt-heading ml-2">
			<a id="btnNew" href="/purchase.php?action=new" class="btn btn-primary">New</a>  
			<a id="btnSave" href="/purchase.php?action=save" class="btn btn-success">Save</a> 
			<a id="btnEmpty" href="/purchase.php?action=empty" class="btn btn-danger">Clear</a>
		</div>
		
	<div class="product-item">

	<table>
		<th class="text-center">SEARCH BY ID</th><th class="text-center">SEARCH BY NAME</th>
		<tr>
			<td align="center">
				<form method="post" action="/purchase.php?action=search">
					<div class="input-group mb-3">
						<input name="itemcode" type="text" class="form-control"  id="itemcode">
						<button type="submit" name="idsubmit" class="btn btn-info mb-2 ml-2">Search ID</button>
					</div>
					<!-- <input type="submit" name="idsubmit" class="form-control btn btn-info mt-1" value="Search ID" /> -->
			...
			<td align="center">
				<form method="post" action="/purchase.php?action=search">
					<div class="input-group mb-2">
						...
						<button type="submit" name="namesubmit" class="btn btn-info mb-2 ml-2">Search Name</button>
					</div>
						<!-- <input type="submit" name="namesubmit" class="btn btn-info mt-1" value="Search Name" /> -->
	...
	$grandtotal=0;
		foreach ($_SESSION["cart_item"] as $item){
			?>
					...
					<td align="center" class="auto-style3"><a href="/purchase.php?action=remove&codetr=<?php echo $item["barcode"]; ?>" class="btn btn-danger mt-1" >Remove Item</a></td>
