<?php
    ...
    // if(isset($_POST['submit'])){
            ...
            $sql = "SELECT wsellhead.s_code,wsellhead.s_date,wcustomers.c_code,wcustomers.c_name,wcustomers.c_addr,wselltail.* FROM wselltail,wsellhead,wcustomers where wsellhead.s_date BETWEEN '$date1' AND '$date2' AND wsellhead.c_code = wcustomers.c_code and wsellhead.s_code = wselltail.s_code";
        ...
        //start while loop to get data
            ...
            {
                $subtotal = $row['i_qty']*$row['i_sell'];
                $subtotaltax = 1+($subtotal*1.1);
                $itemdata = array($row['s_code']=>array('no'=>$row['s_code'],'invdate'=>$row['s_date'],'npwp'=>'0000000','name'=>$row['c_name'],'nik'=>$row['c_code'],'addr'=>$row['c_addr'],'jumdpp'=>$subtotal,'jumppn'=>$subtotaltax,'jumppnbm'=>'0','ide'=>'','fguangmuka'=>'0','uangmukadpp'=>'0','uangmukappn'=>'0','uangmukappnbm'=>'0','referensi'=>'','kodeobjek'=>'A01','namaproduk'=>$row['i_name'],'hargasatuan'=>$row['i_sell'],'jumlahbarang'=>$row['i_qty'],'hargatotal'=>$subtotal,'diskon'=>'0','dpp'=>$subtotal,'ppn'=>$subtotaltax,'tarifppnbm'=>'0','ppnbm'=>'0','dokpendukung'=>'0'));
                $schema_insert = "";
